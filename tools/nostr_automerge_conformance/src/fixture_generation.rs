...
fn generate_many_actor_causal_next_v13(root: &Path) -> Result<(), String> {
    let fixture_id = "many_actor_causal_next_op_exact_budget";
    let controller = Signer::from_byte(179)?;
    let writers = (180..=187)
    ...
    .parse()
    .map_err(|_| "invalid v13 many actor coordinate".to_owned())?;
    let control = sign_control(
        &controller,
        1,
        coordinate,
    ...
    for (index, writer) in writers.iter().enumerate() {
        let actor = ActorId::derive(coordinate, writer.public_key);
        let mut document = AuthoringDocument::empty(ActorState::initial(actor, Default::default()))
            .map_err(|error| format!("v13 many actor document {index}: {error:?}"))?;
        let authored = document
            .author_change(&[Operation::PutString {
                key: format!("actor-{index}"),
                value: "accepted".to_owned(),
            }])
            .map_err(|error| format!("v13 many actor change {index}: {error:?}"))?;
        let (raw, hash) = if let Some(predecessor) = predecessor {
            with_change_dependencies(authored.raw(), &[predecessor], index as u64 + 1)?
        } else {
            (authored.raw().to_vec(), authored.change_hash())
        };
        events.push(sign_change(
            writer,
            index as u64 + 2,
            coordinate,
            control_id,
...
mod tests {
        ...
        for (fixture_id, required) in [
            ...
            let signed = SignedScenarioInput::parse(
                &std::fs::read(root.join(format!("{fixture_id}.input.json")))
                    .expect("checked-in v15 budget-rebound input"),
            )
            .expect("closed v15 budget-rebound input");
            let coordinate = signed.coordinate.parse().expect("v15 coordinate");
            let events = signed
                .raw_events
                .iter()
                .map(|event| {
                    RawEventBytes::new(
                        &event.decoded().expect("v15 Event bytes"),
    ...
    #[test]
    #[allow(clippy::expect_used)]
    fn many_actor_causal_next_op_exact_budget() {
        let fixture_id = "many_actor_causal_next_op_exact_budget";
        let root = repository_root().join("fixtures/v15/rebindings/causal_projection");
        let signed = SignedScenarioInput::parse(
            &std::fs::read(root.join(format!("{fixture_id}.input.json")))
                .expect("checked-in v13 many actor input"),
        )
        .expect("closed v13 many actor input");
        assert_eq!(signed.requirements, ["NCRDT-RESOURCE-018"]);
        let coordinate = signed
            .coordinate
            .parse()
            .expect("v13 many actor coordinate");
        let events = signed
            .raw_events
            .iter()
            .map(|event| {
                RawEventBytes::new(
                    &event.decoded().expect("v13 many actor Event bytes"),
                    ProtocolRevision::draft_v1(),
                )
                .expect("bounded v13 many actor Event")
            })
            .collect::<Vec<_>>();
        ...
        .expect("v13 many actor report");
        let expected = serde_json::from_value::<ExpectedReport>(signed.expected_report)
            .expect("v13 many actor expected report");
        assert_eq!(expected.accepted_changes.len(), 8);
        assert_eq!(
    ...
    fn v15_persistent_fixtures_bind_selected_boundaries_and_eight_orders() {
        for (fixture_id, event_count, control_count, accepted_count, excluded_count) in [
            ...
            let signed = SignedScenarioInput::parse(
                &std::fs::read(root.join(format!("{fixture_id}.input.json")))
                    .expect("checked-in v15 persistent input"),
            )
            .expect("closed v15 persistent input");
            ...
            let coordinate = signed
                ...
                .expect("v15 persistent coordinate");
            let events = signed
                .raw_events
                .iter()
                .map(|event| {
                    RawEventBytes::new(
                        &event.decoded().expect("v15 persistent Event bytes"),
                        ProtocolRevision::draft_v1(),
                    )
                    .expect("bounded v15 persistent Event")
                })
                .collect::<Vec<_>>();
            ...
            .expect("v15 persistent evaluator report");
            let expected = serde_json::from_value::<ExpectedReport>(signed.expected_report)
                .expect("v15 persistent expected report");
            assert_eq!(
            ...
            assert_eq!(
                write_canonical_report(&actual).expect("v15 actual canonical report"),
                write_canonical_report(&expected).expect("v15 expected canonical report"),
                "{fixture_id}"
            );
        }
    }
    ...
    fn exact_resource_fixtures_bind_budget_isolation_and_output_bytes() {
        let cases: [(&str, &str, bool, u64, u64); 4] = [
            ...
            ("scope", "foreign_claim_flood_exact_budget", true, 124, 42),
            (
                "scope",
                "unrelated_valid_checkpoints_exact_budget",
                true,
