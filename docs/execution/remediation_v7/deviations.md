...
The six branch fixtures introduced before the atomic 129-row registry
transition cite existing `NCRDT-CONTROL-001` and `NCRDT-EPOCH-003` rows instead
of noncanonical planning aliases. The signed-fixture schema and repository
...
these transitional citations with `NCRDT-BRANCH-001` and
`NCRDT-BRANCH-002` after step 1082 atomically installs those canonical rows.
