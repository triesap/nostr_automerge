...
def validate(registry: object) -> None:
    ...
    require(registry["schema"] == "nostr_automerge.remediation_v13_reproductions.v1" and registry["status"] == "expected_failure" and registry["result"] == "pass", "registry:state")
    ...
    for index, row in enumerate(cases):
        ...
        require(row["expected"] == "failed_assertion" and row["test"] in source, f"case:{index}:binding")
...
def main() -> int:
    ...
    print(f"PASS: remediation-v13 reproductions cases=2 mutations={mutations} open=2")
    return 0
