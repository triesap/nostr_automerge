...
def validate_ledger(value: object) -> None:
    ...
    if record["cursor"] != {"active_rcld": 101, "active_step": "step_1319", "next_step": "step_1320", "last_planned_step": "step_1363", "remaining_checkpoint_count": 45, "remaining_rcld_count": 8}:
...
def mutation_self_test() -> int:
    ...
    for mutate in (
        lambda value: value["cursor"].update(active_step="step_1320"),
        lambda value: value["cursor"].update(remaining_checkpoint_count=44),
