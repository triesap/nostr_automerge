...
def validate_ledger(value: object) -> None:
    ...
    if record["cursor"] != {"active_rcld": 101, "active_step": "step_1316", "next_step": "step_1317", "last_planned_step": "step_1363", "remaining_checkpoint_count": 48, "remaining_rcld_count": 8}:
...
def mutation_self_test() -> int:
    ...
    for mutate in (
        lambda value: value["cursor"].update(active_step="step_1317"),
        lambda value: value["cursor"].update(remaining_checkpoint_count=47),
