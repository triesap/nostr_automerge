...
def validate_ledger(value: object) -> None:
    ...
    if record["cursor"] != {"active_rcld": 102, "active_step": "step_1324", "next_step": "step_1325", "last_planned_step": "step_1363", "remaining_checkpoint_count": 40, "remaining_rcld_count": 7}:
...
def mutation_self_test() -> int:
    ...
    for mutate in (
        lambda value: value["cursor"].update(active_step="step_1325"),
        lambda value: value["cursor"].update(remaining_checkpoint_count=39),
