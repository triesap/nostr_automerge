...
from validate_interop_attestation_v4 import validate_current as validate_interop
...
def validate(report: dict[str, object]) -> None:
    ...
    if set(report) != required or report.get("schema") != "nostr_automerge.requirement_coverage.v5":
    ...
    if report.get("requirement_count") != 96 or not isinstance(rows, list) or [row.get("id") for row in rows] != [item["id"] for item in requirements]:
        raise EvidenceError("missing_duplicate_unknown_or_reordered")
    manifest = load("reports/test_evidence_manifest_v5.json")
    ...
    attestation = load("reports/interop_typescript_v4.json")
    validate_interop()
    overlay_report = load("reports/requirements_typescript_overlay_v5.json")
...
def self_test(baseline: dict[str, object]) -> None:
    ...
    output = {"schema": "nostr_automerge.requirement_evidence_mutations.v5", "generated": len(caught), "caught": len(caught), "survived": 0, "status": "pass", "mutations": caught}
    (ROOT / "reports/requirements_evidence_mutations_v5.json").write_text(json.dumps(output, sort_keys=True, separators=(",", ":")) + "\n")
...
def main() -> int:
    ...
    schema = load("tools/validation/requirement_coverage_v5.schema.json")
    if schema.get("$schema") != "https://json-schema.org/draft/2020-12/schema":
        raise EvidenceError("schema_version")
    report = load("reports/requirements_coverage_v5.json")
    validate(report)
    if args.self_test:
        self_test(report)
    print("PASS: all 96 final requirement rows are exact, current, and fail closed")
    return 0
