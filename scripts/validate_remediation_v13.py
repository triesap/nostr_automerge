...
CLOSED = [identifier for identifier in ALL if identifier != "FINDING_111"]
OPEN = ["FINDING_111"]
SCOPE = ["docs/execution/remediation_v13/ledger.md","implementation/runtime_ledger_v13.json","reports/causal_projection_combined_assurance_v14.json","reports/spec_baseline.txt","scripts/validate_causal_projection_combined_assurance_v14.py","scripts/validate_private_reproduction_boundary_v9.py","scripts/validate_remediation_v13.py","scripts/validate_spec.py","tools/nostr_automerge_xtask/src/validate.rs","tools/validation/causal_projection_combined_assurance_v14.schema.json"]
...
def validate(authority: object, findings: object, ledger: object, schema: object, evidence: object, evidence_schema: object) -> None:
    ...
    require(all(row["status"] == ("open" if row["id"] == "FINDING_111" else "closed") for row in f["findings"][:-1]) and f["findings"][-1]["status"] == "held", "findings:status")
    ...
    require(l["cursor"] == {"active_rcld":120,"active_step":"step_1449","next_step":"step_1450","last_planned_step":"step_1452","remaining_checkpoint_count":4,"remaining_rcld_count":1}, "ledger:cursor")
    ...
    require(latest_predecessor == {"step":"step_1448","candidate":"89ccc8af6de5d0f593da32b537fc12cf2d9610b1","owner_class":"public","result":"pass"}, "ledger:latest_predecessor")
    ...
    l["predecessors"].extend((prior_predecessor, latest_predecessor))
...
def self_test(authority: dict, findings: dict, ledger: dict, schema: dict, evidence: dict, evidence_schema: dict) -> int:
    ...
    for label, target, mutate in [
        ...
        ("premature_close","findings",lambda value: value["findings"][7].update(status="closed")),
...
def main() -> int:
    ...
    print(f"PASS: remediation-v13 authority active=step_1449 open=1 closed=8 mutations={mutations}")
    return 0
