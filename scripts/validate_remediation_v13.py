...
CLOSED = ALL
OPEN = []
SCOPE = ["docs/execution/rcl/nostr_automerge_v1_multi_rcld_v13.md","docs/execution/remediation_v13/ledger.md","implementation/runtime_ledger_v13.json","reports/causal_projection_final_decision_v14.json","reports/spec_baseline.txt","scripts/validate_causal_projection_final_decision_v14.py","scripts/validate_causal_projection_finding_closure_v14.py","scripts/validate_private_reproduction_boundary_v9.py","scripts/validate_remediation_v13.py","scripts/validate_spec.py","spec/remediation_findings_v13.json","spec/remediation_v13_authority.json","tools/nostr_automerge_xtask/src/validate.rs","tools/validation/causal_projection_final_decision_v14.schema.json"]
...
def validate(authority: object, findings: object, ledger: object, schema: object, evidence: object, evidence_schema: object) -> None:
    ...
    require(a["schema"] == "nostr_automerge.remediation_v13_authority.v1" and a["status"] == "code_complete_publication_held" and a["result"] == "pass", "authority:state")
    ...
    require(a["governing_plan"] == {"path":"docs/execution/rcl/nostr_automerge_v1_multi_rcld_v13.md","sha256":"4166a0a2710a364ca960e1fcda80b1b48e9b3073905bf8543cba607bf726a4e8"}, "authority:plan")
    ...
    require(f["schema"] == "nostr_automerge.remediation_findings.v13.v1" and f["status"] == "code_complete_publication_held" and f["result"] == "pass", "findings:state")
    ...
    require(l["schema"] == "nostr_automerge.runtime_ledger.v13.v1" and l["status"] == "code_complete_publication_held", "ledger:state")
    require(l["authority"] == "spec/remediation_v13_authority.json", "ledger:authority")
    require(l["cursor"] == {"active_rcld":120,"active_step":"step_1452","next_step":None,"last_planned_step":"step_1452","remaining_checkpoint_count":0,"remaining_rcld_count":0}, "ledger:cursor")
    ...
    require(latest_predecessor == {"step":"step_1451","candidate":"bc0ac22fe4645c3acb5d25730e6be16045813f27","owner_class":"public","result":"pass"}, "ledger:latest_predecessor")
    recent_predecessor = l["predecessors"].pop()
    require(recent_predecessor == {"step":"step_1450","candidate":"ec9c8d7d40242eeec1bcabd2ea484d25268f3f9a","owner_class":"public","result":"pass"}, "ledger:recent_predecessor")
    ...
    l["predecessors"].extend((earlier_predecessor, prior_predecessor, middle_predecessor, recent_predecessor, latest_predecessor))
...
def main() -> int:
    ...
    print(f"PASS: remediation-v13 authority active=step_1452 complete=true open=0 closed=9 mutations={mutations}")
    return 0
