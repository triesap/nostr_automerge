...
def committed(path: str) -> bytes:
    run = subprocess.run(["git", "show", f"{BASE}:{path}"], cwd=ROOT, capture_output=True)
    require(run.returncode == 0, "binding:" + path)
    return run.stdout
def validate(record: dict[str, Any], schema: dict[str, Any]) -> None:
    require(list(record) == FIELDS and record["schema"] == "nostr_automerge.causal_projection_finding_closure.v19.v1" and record["status"] == "verified" and record["rcld"] == 146 and record["base_candidate"] == BASE and record["result"] == "pass", "state")
    finding_registry = committed("spec/remediation_findings_v19.json")
    runtime_ledger = committed("implementation/runtime_ledger_v19.json")
    require(record["imports"] == {"finding_registry_sha256": hashlib.sha256(finding_registry).hexdigest(), "runtime_ledger_sha256": hashlib.sha256(runtime_ledger).hexdigest(), "combined_assurance_sha256": sha(ROOT / "reports/causal_projection_combined_assurance_v19.json")}, "imports")
    registry = json.loads(finding_registry)
