...
def validate(authority: Any, findings: Any, ledger: Any, schema: Any) -> None:
    ...
    require(
        l["active_checkpoint_scope"] == STEP_1472_SCOPE
        and STEP_1472_SCOPE == sorted(STEP_1472_SCOPE)
        and all((ROOT / path).exists() for path in STEP_1472_SCOPE),
        "ledger:scope",
    )
    require(
        l["predecessors"]
...
def self_test(authority: Any, findings: Any, ledger: Any, schema: Any) -> int:
    mutations = [
        ...
        ("cursor", "ledger", lambda value: value["cursor"].update(next_step="step_1474")),
...
def main() -> int:
    ...
    print(
        "PASS: remediation-v16 active=step_1472 next=step_1473 "
