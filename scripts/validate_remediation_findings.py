...
def main() -> int:
    ...
    if value["schema"] != "nostr_automerge.remediation_findings.v1" or ids != expected:
        raise AssertionError("finding IDs or schema are invalid")
    if len(ids) != len(set(ids)) or any(item["status"] != "open" for item in findings):
        raise AssertionError("findings must be unique and initially open")
    ...
    print("PASS: 13 unique remediation findings are open and traceable")
    return 0
