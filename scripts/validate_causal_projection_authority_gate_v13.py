...
AUTHORITY_GATE_CANDIDATE = "fbb3fd31bd0d37ff4976f733aa574e185d5280b6"
REPRODUCTIONS_PATH = "spec/remediation_v13_reproductions.json"
...
CANDIDATES = [
    ...
    {"step":"step_1423","candidate":"285c3922c7a7d80af361bfa011b223caca43e3e1"},
...
REVIEWED_PREDECESSOR = "00ef954ff2dece37119ad235638046ffaa7305d4"
...
def historical_json(candidate: str, path: str) -> object:
    ...
    require(result.returncode == 0 and result.stderr == "", "historical:read")
...
def validate(report: object, schema: object, contract: object, reproductions: object) -> None:
    ...
    for index, row in enumerate(CANDIDATES):
        actual = subprocess.run(["git","rev-parse","--verify",f"{row['candidate']}^{{commit}}"],cwd=ROOT,capture_output=True,text=True,check=False)
        require(actual.returncode == 0 and actual.stdout.strip() == row["candidate"], f"candidate:{index}")
        parent = subprocess.run(["git","rev-parse","--verify",f"{row['candidate']}^"],cwd=ROOT,capture_output=True,text=True,check=False)
        require(parent.returncode == 0 and parent.stdout.strip() == prior, f"candidate:{index}:parent")
        prior = row["candidate"]
...
def main() -> int:
    ...
    reproductions = historical_json(AUTHORITY_GATE_CANDIDATE, REPRODUCTIONS_PATH)
