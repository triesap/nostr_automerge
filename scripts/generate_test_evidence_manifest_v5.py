#!/usr/bin/env python3
"""Generate exact commit-bound Rust test and signed-fixture evidence v5."""
...
OUTPUT = ROOT / "reports/test_evidence_manifest_v5.json"
...
def main() -> int:
    ...
    parser.add_argument("--profile-root", type=Path, default=ROOT / ".local/evidence-v5/rust")
    ...
    if len(fixtures) != 124 or len(fixtures) != len(set(fixtures)):
        raise AssertionError("invalid signed-v5 fixture inventory")
    ...
    print(f"PASS: exact v5 evidence contains {len(tests)} tests and {len(fixtures)} fixtures")
    return 0
