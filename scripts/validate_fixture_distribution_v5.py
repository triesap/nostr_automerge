...
def _validate_manifest(root: Path, manifest: dict[str, object]) -> None:
    ...
    if manifest.get("requirements_sha256") != hashlib.sha256(requirements.read_bytes()).hexdigest():
        raise SystemExit("fixture distribution requirements hash is stale")
    if manifest.get("authority_sha256") != hashlib.sha256(authority.read_bytes()).hexdigest():
        raise SystemExit("fixture distribution authority hash is stale")
    if manifest.get("supersedes") != "fixtures/distribution/manifest_v4.json":
        raise SystemExit("fixture distribution does not supersede v4")
...
def self_test() -> None:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        spec = root / "spec"
        spec.mkdir()
        spec.joinpath("requirements.json").write_bytes(b"requirements")
        spec.joinpath("NOSTR_AUTOMERGE_V1_SPEC.md").write_bytes(b"authority")
