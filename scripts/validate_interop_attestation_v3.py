#!/usr/bin/env python3
"""Fail closed on stale, altered, or leaky interoperability evidence v3."""
...
def validate_attestation(value: dict[str, object], language: str) -> None:
    ...
    schema = (
        ...
        else "nostr_automerge.rust_interop_attestation.v3"
    )
    identity = (
...
def validate_current() -> None:
    rust_path = REPORTS / "interop_rust_v3.json"
    typescript_path = REPORTS / "interop_typescript_v3.json"
    combined_path = REPORTS / "interop_combined_v3.json"
    rust = load(rust_path)
    ...
    manifest_hash = sha256(ROOT / "fixtures/distribution/manifest_v4.json")
    ...
    if combined["schema"] != "nostr_automerge.interop_combined.v3":
...
def self_test() -> list[dict[str, str]]:
    baseline_path = REPORTS / "interop_typescript_v3.json"
...
def main() -> int:
    ...
    if args.self_test:
        caught = self_test()
        output = {
            "schema": "nostr_automerge.interop_evidence_mutations.v3",
        ...
        (REPORTS / "interop_evidence_mutations_v3.json").write_text(
            json.dumps(output, sort_keys=True, separators=(",", ":")) + "\n",
            encoding="utf-8",
        )
    print("PASS: final interoperability evidence v3 is exact, current, and source-free")
    return 0
